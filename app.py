...
def get_agent():
    ...
    return Agent(
        ...
        instructions=[
            ...
            "1. Every sprite script MUST start with a hat block (event_whenflagclicked) with topLevel: true and parent: null",
            "2. All blocks need 'parent' field pointing to their container block (the block that references them)",
            "3. CONDITION inputs must use [2, 'block_id'] format referencing a separate condition block, NOT inline objects",
            "4. sensing_keypressed MUST have KEY_OPTION in 'fields' not 'inputs': {\"KEY_OPTION\": [\"space\", null]}",
